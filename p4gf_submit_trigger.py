...
def fix_text_encoding(text):
    ...
    if not PYTHON3:
        try:
            ...
            # If text itself is unicode, the unicode() will fail
            # thus text is already unicode
            pass
    try:
...
# should typically be avoided.
    ...
    P4GF_DEPOT_NAME = 'P4GF_DEPOT'
    P4GF_P4KEY_P4GF_DEPOT = 'git-fusion-p4gf-depot'
    P4GF_REQ_PATCH = {
        2015.1: 1171507,
        2015.2: 1171507
    }
...
#                 and p4gf_submit_trigger.py.
#
    P4GF_P4KEY_LOCK_VIEW = 'git-fusion-view-{repo_name}-lock'
    P4GF_P4KEY_LOCK_VIEW_OWNERS = 'git-fusion-view-{repo_name}-lock-owners'
#
#                 End block copied to both p4gf_const.py
#                 and p4gf_submit_trigger.py.
# -----------------------------------------------------------------------------
...
#
    P4GF_REVIEWS_GF = 'git-fusion-reviews-'  # Append GF server_id.
    P4GF_REVIEWS__NON_GF = P4GF_REVIEWS_GF + '-non-gf'
    P4GF_REVIEWS__ALL_GF = P4GF_REVIEWS_GF + '-all-gf'
    P4GF_REVIEWS_NON_GF_SUBMIT = 'git-fusion-non-gf-submit-'
    P4GF_REVIEWS_NON_GF_RESET = 'git-fusion-non-gf-'
    DEBUG_P4GF_REVIEWS__NON_GF = 'DEBUG-' + P4GF_REVIEWS__NON_GF
    DEBUG_SKIP_P4GF_REVIEWS__NON_GF = 'DEBUG-SKIP-' + P4GF_REVIEWS__NON_GF
    ...
    P4GF_REVIEWS_COMMON_LOCK = 'git-fusion-reviews-common-lock'
    P4GF_REVIEWS_COMMON_LOCK_OWNER = 'git-fusion-reviews-common-lock-owner'
    P4GF_REMAPPED = '//{P4GF_DEPOT}/remapped/{repo_name}'

# Is the Atomic Push submit trigger installed and at the correct version?
#
    P4GF_P4KEY_TRIGGER_VERSION = 'git-fusion-submit-trigger-version'
    P4GF_TRIGGER_VERSION = '2017.2-001'
    P4GF_P4KEY_LOCK_USER = 'git-fusion-user-{user_name}-lock'
...
MSG_USAGE = _("""
    ...
    *) The update of the trigger spec be done automatically by running the
    ...
    This removes all 'p4 reviews' and 'p4 keys' data stored by this trigger
    and Git Fusion used to provide atomic locking for 'git push'. In
    particular, it removes those Reviews entries that pertain to changes
    ...
    leaving the other reviews and keys in place, if any.
        ...
        python p4gf_submit_trigger.py --unset P4PORT [superuser]

    The 'unset' option completely removes everything from the Reviews field.
...
ACTION_RESET_SAFE = NTR('reset-safe')
...
Usage:
    ...
    p4gf_submit_trigger.py --unset [--git-fusion-server=SERVER_ID] P4PORT [superuser]
...
#                 and p4gf_submit_trigger.py.
# [repo-creation]
    SECTION_REPO_CREATION = 'repo-creation'
    KEY_CHARSET = 'charset'
    KEY_GIT_AUTOPACK = 'git-autopack'
    KEY_GIT_GC_AUTO = 'git-gc-auto'
    KEY_NDPR_ENABLE = 'depot-path-repo-creation-enable'
    KEY_NDPR_P4GROUP = 'depot-path-repo-creation-p4group'

# [git-to-perforce]
    SECTION_GIT_TO_PERFORCE = 'git-to-perforce'
    KEY_CHANGE_OWNER = 'change-owner'
    KEY_ENABLE_BRANCH_CREATION = 'enable-git-branch-creation'
    KEY_ENABLE_MERGE_COMMITS = 'enable-git-merge-commits'
    KEY_ENABLE_SWARM_REVIEWS = 'enable-swarm-reviews'
    KEY_ENABLE_SUBMODULES = 'enable-git-submodules'
    KEY_PREFLIGHT_COMMIT = 'preflight-commit'
    KEY_PREFLIGHT_PUSH = 'preflight-push'
    KEY_IGNORE_AUTHOR_PERMS = 'ignore-author-permissions'
    KEY_READ_PERMISSION_CHECK = 'read-permission-check'
    KEY_GIT_MERGE_AVOIDANCE_AFTER_CHANGE_NUM = 'git-merge-avoidance-after-change-num'
    KEY_ENABLE_GIT_FIND_COPIES = 'enable-git-find-copies'
    KEY_ENABLE_GIT_FIND_RENAMES = 'enable-git-find-renames'
    KEY_ENABLE_FAST_PUSH = 'enable-fast-push'
    KEY_JOB_LOOKUP = 'job-lookup'
    KEY_NDB_ENABLE = 'depot-branch-creation-enable'
    VALUE_NDB_ENABLE_NO = 'no'
    VALUE_NDB_ENABLE_EXPLICIT = 'explicit'
    VALUE_NDB_ENABLE_ALL = 'all'
    KEY_NDB_P4GROUP = 'depot-branch-creation-p4group'
    KEY_NDB_DEPOT_PATH = 'depot-branch-creation-depot-path'
    VALUE_NDB_DEPOT_PATH_DEFAULT = '//depot/{repo}/{git_branch_name}'
    KEY_NDB_VIEW = 'depot-branch-creation-view'
    VALUE_NDB_VIEW_DEFAULT = '... ...'
    KEY_FAST_PUSH_WORKING_STORAGE = 'fast-push-working-storage'
    KEY_USE_SHA1_TO_SKIP_EDIT = 'use-sha1-to-skip-edit'
    KEY_ENFORCE_LW_PROTECTIONS = 'enforce-lw-protections'
    KEY_FIX_KTEXT_DIGESTS = 'fix-ktext-digests'
    KEY_CHANGELIST_DATE_SOURCE = 'changelist-date-source'
    VALUE_DATE_SOURCE_P4_SUBMIT = 'perforce-submit-time'
    KEY_LABEL_FOR_COMMIT_TAG = 'create-label-for-commit-tag'
#                 End block copied to both p4gf_config.py
#                 and p4gf_submit_trigger.py.
    KEY_MIRROR_MAX_COMMITS_PER_SUBMIT = "gitmirror-max-commits-per-submit"
#                 Begin block copied to both p4gf_config.py
#                 and p4gf_submit_trigger.py.
    SECTION_PERFORCE_TO_GIT = 'perforce-to-git'
    KEY_SUBMODULE_IMPORTS = 'enable-stream-imports'
    KEY_CLONE_TO_CREATE_REPO = 'enable-clone-to-create-repo'
    KEY_UPDATE_ONLY_ON_POLL = 'update-only-on-poll'
    KEY_HTTP_URL = 'http-url'
    KEY_SSH_URL = 'ssh-url'
    KEY_ENABLE_ADD_COPIED_FROM_PERFORCE = 'enable-add-copied-from-perforce'
    KEY_ENABLE_GIT_P4_EMULATION = 'enable-git-p4-emulation'
    KEY_ENABLE_CHERRY_PICK = 'enable-cherry-pick'
    KEY_GIT_LFS_ENABLE = 'git-lfs-enable'
    KEY_GIT_LFS_INITIAL_TRACK = 'git-lfs-initial-track'
    KEY_FAST_RECLONE_ENABLE = 'enable-fast-reclone'
# [@features]
...
# [authentication]
    ...
    KEY_EMAIL_CASE_SENSITIVITY = 'email-case-sensitivity'
    KEY_AUTHOR_SOURCE = 'author-source'
# [quota]
    SECTION_QUOTA = 'quota'
    KEY_COMMIT_LIMIT = 'limit_commits_received'
    KEY_FILE_LIMIT = 'limit_files_received'
    KEY_SPACE_LIMIT = 'limit_space_mb'
    KEY_RECEIVED_LIMIT = 'limit_megabytes_received'

# [undoc]
    SECTION_UNDOC = 'undoc'
    KEY_ENABLE_CHECKPOINTS = 'enable_checkpoints'
#
# In [@repo] of the per-repo config files only
#
    SECTION_REPO = '@repo'
    KEY_DESCRIPTION = 'description'
    KEY_FORK_OF_REPO = 'fork-of-repo'
    KEY_LW_NON_FF_ENABLE = 'lw-non-ff-push-enable'
# This next key may appear as a branch rather than a repo option
    KEY_FP_NON_FF_ENABLE = 'fp-non-ff-push-enable'
#
# The following may also appear in the branch section...
#
    KEY_READ_ONLY = 'read-only'
    KEY_FORK_OF_BRANCH_ID = 'fork-of-branch-id'

#
# In the [<branch_id>] section of the per-repo config files only
#
    KEY_GIT_BRANCH_NAME = 'git-branch-name'
    KEY_VIEW = 'view'
    KEY_STREAM = 'stream'
    KEY_STREAMDEF_START_CL = 'streamdef-start-cl'
    KEY_ORIGINAL_VIEW = 'original-view'
    KEY_ENABLE_MISMATCHED_RHS = 'enable-mismatched-rhs'
    KEY_GIT_BRANCH_DELETED = 'deleted'
    KEY_GIT_BRANCH_DELETED_CHANGE = 'deleted-at-change'
    KEY_GIT_BRANCH_START_CHANGE = 'start-at-change'
    KEY_DEPOT_BRANCH_ID = 'depot-branch-id'
    KEY_ALLOW_ISOLATE_PATHS = 'allow-isolate-paths'
#                 End block copied to both p4gf_config.py
#                 and p4gf_submit_trigger.py.
# -----------------------------------------------------------------------------
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
    ...
    def default_config_global():
        ...
        config.set(p4gf_config.SECTION_GIT_TO_PERFORCE,
                p4gf_config.KEY_PREFLIGHT_PUSH, '')
        config.set(p4gf_config.SECTION_GIT_TO_PERFORCE,
                p4gf_config.KEY_IGNORE_AUTHOR_PERMS, p4gf_config.VALUE_NO)
        config.set(p4gf_config.SECTION_GIT_TO_PERFORCE,
                p4gf_config.KEY_ENFORCE_LW_PROTECTIONS, p4gf_config.VALUE_NO)
        config.set(p4gf_config.SECTION_GIT_TO_PERFORCE,
                p4gf_config.KEY_READ_PERMISSION_CHECK, '')
        config.set(p4gf_config.SECTION_GIT_TO_PERFORCE,
                p4gf_config.KEY_GIT_MERGE_AVOIDANCE_AFTER_CHANGE_NUM, '0')
        config.set(p4gf_config.SECTION_GIT_TO_PERFORCE,
        ...
        config.set(p4gf_config.SECTION_GIT_TO_PERFORCE,
                p4gf_config.KEY_LABEL_FOR_COMMIT_TAG, p4gf_config.VALUE_NO)
        config.set(p4gf_config.SECTION_GIT_TO_PERFORCE,
                p4gf_config.KEY_LABEL_FOR_COMMIT_TAG, p4gf_config.VALUE_NO)
        config.set(p4gf_config.SECTION_GIT_TO_PERFORCE,
                p4gf_config.KEY_MIRROR_MAX_COMMITS_PER_SUBMIT, '10000')
        config.set(p4gf_config.SECTION_GIT_TO_PERFORCE,
                p4gf_config.KEY_USE_SHA1_TO_SKIP_EDIT, p4gf_config.VALUE_YES)
        config.set(p4gf_config.SECTION_GIT_TO_PERFORCE,
                p4gf_config.KEY_FIX_KTEXT_DIGESTS, p4gf_config.VALUE_YES)
        ...
        config.set(p4gf_config.SECTION_PERFORCE_TO_GIT,
            p4gf_config.KEY_FAST_RECLONE_ENABLE, p4gf_config.VALUE_YES)
        config.set(p4gf_config.SECTION_PERFORCE_TO_GIT,
        ...
        config.set(p4gf_config.SECTION_PERFORCE_TO_GIT,
            p4gf_config.KEY_ENABLE_CHERRY_PICK, p4gf_config.VALUE_NO)
...
class Branch:
    ...
    def from_config(config, branch_id, strict=False):
        ...
        if strict and not (config.has_option(branch_id, p4gf_config.KEY_STREAM)
            ...
            return (None, f.format(section=branch_id))
        view_lines = None
        if config.has_option(branch_id, p4gf_config.KEY_ALLOW_ISOLATE_PATHS):
            isolates_allowed = config.getboolean(
                branch_id, p4gf_config.KEY_ALLOW_ISOLATE_PATHS)
        else:
            isolates_allowed = False
        ...
        if config.has_option(branch_id, p4gf_config.KEY_STREAM):
            ...
            if not isolates_allowed and stream_contains_isolate_path(stream):
...
INVALID_GIT_BRANCH_NAME_SUFFIXES = ['/', '.', '.lock']
INVALID_GIT_BRANCH_NAME_SUBSTR = ['..', '^', '*', '~', '?', ':', '//', '[', ' ', '\\']
...
class Validator:
    ...
    def is_valid(self):
        ...
        # Make sure if a fp-non-ff-enable specified that it's valid
        if self.config.has_option(p4gf_config.SECTION_REPO, p4gf_config.KEY_FP_NON_FF_ENABLE):
            try:
                self.config.getboolean(p4gf_config.SECTION_REPO, p4gf_config.KEY_FP_NON_FF_ENABLE)
            except ValueError:
                self.report_error(_("repository configuration option '{key}' has illegal value\n")
                                   .format(key=p4gf_config.KEY_FP_NON_FF_ENABLE))
                return False
        # Make sure if a lw-non-ff-enable specified that it's valid
        if self.config.has_option(p4gf_config.SECTION_REPO, p4gf_config.KEY_LW_NON_FF_ENABLE):
            try:
                self.config.getboolean(p4gf_config.SECTION_REPO, p4gf_config.KEY_LW_NON_FF_ENABLE)
            except ValueError:
                self.report_error(_("repository configuration option '{key}' has illegal value\n")
                                   .format(key=p4gf_config.KEY_LW_NON_FF_ENABLE))
                return False
        ...
        # Validate the preflight-push option
        section_name = p4gf_config.SECTION_GIT_TO_PERFORCE
        if self.config.has_section(p4gf_config.SECTION_REPO):
            section_name = p4gf_config.SECTION_REPO
        if self.config.has_option(section_name, p4gf_config.KEY_PREFLIGHT_PUSH):
            value = self.config.get(section_name, p4gf_config.KEY_PREFLIGHT_PUSH)
            if not self.validate_preflight_commit_value(value):
                self.report_error(_("invalid preflight-push value: '{value}'\n")
                                  .format(value=value))
                return False
    ...
    def _validate_new_depot_branch(self):
        ...
        if config.has_option(p4gf_config.SECTION_REPO,
            ...
            value = config.get(p4gf_config.SECTION_REPO,
                                   p4gf_config.KEY_NDB_DEPOT_PATH)
            if not str(value).startswith("//") or str(value).endswith("..."):
                self.report_error(_("Perforce: Improperly configured {option} value\n")
    ...
    def _valid_branches(self):
        ...
        for section in sections:
            try:
                ...
                # three checks for invalid chars in git branch name
                        ...
                        self.report_error(
                            _("repository configuration section [{section}]: "
                              ...
                              "start with '{prefix}'").format(
                                ...
                                prefix=prefix))
                        return False
                for suffix in INVALID_GIT_BRANCH_NAME_SUFFIXES:
                    if branch.git_branch_name.endswith(suffix):
                        self.report_error(_("repository configuration section [{section}]: "
                                          "'git-branch-name = {git_branch_name}'"
                                          " contains invalid characters")
                                        .format(section=section,
                                                git_branch_name=branch.git_branch_name))
                        return False
                for substr in INVALID_GIT_BRANCH_NAME_SUBSTR:
                    if substr in branch.git_branch_name:
                        self.report_error(_("repository configuration section [{section}]: "
                                          "'git-branch-name = {git_branch_name}'"
                                          " contains invalid characters")
                                        .format(section=section,
            ...
            try:
                config.has_option(section, p4gf_config.KEY_READ_ONLY) and \
                config.getboolean(section, p4gf_config.KEY_READ_ONLY)
            except ValueError:
                self.report_error(_("repository configuration section [{section}] "
            ...
            # check FP_NON_FF_ENABLE option
            try:
                config.has_option(section, p4gf_config.KEY_FP_NON_FF_ENABLE) and \
                config.getboolean(section, p4gf_config.KEY_FP_NON_FF_ENABLE)
            except ValueError:
                self.report_error(_("repository configuration section [{section}] "
                                    "configuration option '{key}' "
                                     "has illegal value\n")
                                   .format(section=section, key=p4gf_config.KEY_FP_NON_FF_ENABLE))
...
#                return False
            ...
            for lhs in branch.lhs:
                self.lhs.add(lhs)
            # Save each valid branch_id in the branch_dict
    ...
    def _view_valid_depots(self, depot_list, branch_id, lhs):
        ...
        remapped_path = p4gf_const.P4GF_REMAPPED.format(
            P4GF_DEPOT=p4gf_const.P4GF_DEPOT, repo_name="")
        for line in lhs:
            ...
            if depot_name not in referenced_depot_name_list:
                if line.startswith(remapped_path):
                    # The remapped files are allowed.
                    continue
    ...
    def _ignored_options(self):
        ...
        all_options.add(p4gf_config.KEY_FP_NON_FF_ENABLE)
        all_options.add(p4gf_config.KEY_LW_NON_FF_ENABLE)
        ...
        # Check that the repo config does not have any options in supported
        # sections that would be ignored.
        for section in default_cfg.sections():
            if self.config.has_section(section):
                for option in self.config.options(section):
                    if option not in all_options:
                        self.report_error(frm.format(section=section, option=option))
                        has_ignored_options = True
...
def _encoding_list():
    ...
    """
    # latin_1 decodes anything, so keep it last in the list.
    return ['utf8', 'latin_1']
...
def set_p4key(name, value, user=P4GF_USER):
    """Set p4key."""
    p4_run(['key', name, value], user=user)
...
def get_reviews_using_filelist(files, ofile=None):
    ...
    users = p4_run(cmd)
    if DEBUG_LOG_FILE:
        debug_log("Reviews: {0}".format(users))
    ...
    for user in users:
        ...
        if _user.startswith(p4gf_const.P4GF_REVIEWS_GF):
            if DEBUG_LOG_FILE:
                debug_log("reviews user: {0}".format(_user))
            if _user == p4gf_const.P4GF_REVIEWS__ALL_GF:
                ...
                break
    if DEBUG_LOG_FILE:
        debug_log("locked: {0}, in_union: {1}".format(is_locked, change_is_in_union))
...
def update_userspec(userspec, user, p4user=P4GF_USER):
    ...
    for key, val in userspec.items():
        if key == 'Reviews':
            # Use a list comprehension for better performance (see GF-3042).
            reviews = '\n' + key + ':\n' + \
...
def reset_reviews(user, p4user=P4GF_USER):
    """Remove reviews that pertain to submitted changes."""
    update_reviews(user, None, None, action=ACTION_RESET_SAFE, p4user=p4user)
...
def update_reviews(user, depot_files, change, action=ACTION_ADD, p4user=P4GF_USER):
    ...
    if action == ACTION_UNSET:
        newspec['Reviews'] = '\n'
    elif action == ACTION_RESET_SAFE:
        pending_reviews = remove_submitted_reviews(current_reviews)
...
def remove_submitted_reviews(current_reviews):
    """Remove those reviews from the given list that have been submitted.

    :param list current_reviews: list of current reviews (from Reviews field).
    :return: list of reviews that should be retained.

    """
    results = []
    begin_regex = re.compile(p4gf_const.NON_GF_REVIEWS_BEGIN_MARKER_PATTERN.format(r'(\d+)'))
    lines = iter(current_reviews)
    while True:
        try:
            line = next(lines)
            ...
            if match:
                ...
                change = p4_run(['change', '-o', changelist], exit_on_error=False)[0]
                if change.get('Status', None) == "submitted":
                    # skip to the end of the review marker
                    end_marker = p4gf_const.NON_GF_REVIEWS_END_MARKER_PATTERN.format(changelist)
                    while True:
                        line = next(lines)
                        if line == end_marker:
                            break
...
def validate_p4gf_config(change, config_files, add_action=None, user=P4GF_USER):
    ...
    # pylint: disable=too-many-branches
    if DEBUG_LOG_FILE:
        debug_log("validate_p4gf_config: number of configs={0}".format(len(config_files)))
    returncode = P4PASS
...
def rebuild_all_gf_reviews(user=P4GF_USER):
    ...
    p4key_lock_acquired = False
    try:
        acquire_p4key_reviews_common_lock(user=user)
        p4key_lock_acquired = True
        ...
        if p4key_lock_acquired:
            release_p4key_reviews_common_lock(user=user)
    return returncode
...
def acquire_p4key_lock(key_name, max_tries=0, fail_msg=None, user=P4GF_USER):
    """Acquire a p4key based lock."""
    count = 0
    while True:
        if DEBUG_LOG_FILE:
            debug_log("acquire_p4key_lock: key={0}".format(key_name))
        if get_p4key_lock(key_name, user=user):
            break
        count = count + 1
...
def release_p4key_lock(key_name, user=P4GF_USER):
    """Release a p4key based lock."""
    delete_p4key(key_name, user=user)
...
def release_p4key_reviews_common_lock(user=P4GF_USER):
    """Release the Reviews Common Lock."""
    delete_p4key(p4gf_const.P4GF_REVIEWS_COMMON_LOCK_OWNER, user=user)
    release_p4key_lock(p4gf_const.P4GF_REVIEWS_COMMON_LOCK, user=user)
...
def reset_all(user, unset=False):
    ...
    for user_name in gf_reviews_user_name_list():
        if user_name != p4gf_const.P4GF_REVIEWS__ALL_GF:   # preserve the all-gf reviews
            if unset:
                ...
                reset_reviews(user_name, p4user=user)
        release_p4key_user_lock(user_name)
    release_p4key_reviews_common_lock()


def reset_one(p4user, unset=False):
    ...
    if unset:
        ...
        reset_reviews(username, p4user=p4user)
...
def get_user_from_args(option_args, super_user_index=None, super_passwd_index=None):
    ...
    if super_user:
        if super_passwd:
            # If the passwd arg is actually a TICKET
            # rather than attempt login, set the P4PASSWD in the env
            if re.search('^[0-9A-Z]{32}', super_passwd):
                os.environ['P4PASSWD'] = super_passwd
            else:
...
def parse_argv():
    ...
    args = Args()
    short_opt = 'h'
    long_opt = NTR(['reset', 'unset', 'rebuild-all-gf-reviews',
    ...
    if options_len > 1 and options[0][0] != '--install':
        ...
        elif opt == "--unset" and validate_option_or_exit(1, 2, positional_len):
            args.unset = positional
            args.port = args.unset[0]
...
def main():
    ...
    if not args.optional_command:
        ...
        if args.set_version_p4key:
            ...
            # Check if an optional user arg was passed and whether it is a super user
            user = get_user_from_args(args.unset, super_user_index=1)
            _set_p4gf_depot_from_p4key(user=user)
            if SERVER_ID_NAME:
                reset_one(user, unset=True)
            else:
                # Remove all the p4keys and reviews to reset
                reset_all(user, unset=True)
